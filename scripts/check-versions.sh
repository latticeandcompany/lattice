...
#   packages/npm/package.json  version   (and its lockfile)
...
# The npm wrapper's optionalDependencies are not checked here: they name a version
# that does not exist on the registry until the release is published, so they are
# generated by scripts/stage.mjs rather than committed. There is nothing to drift.
v="$(json_field packages/npm/package.json version)"
if [ "$v" = "$CARGO_VERSION" ]; then good "packages/npm/package.json version = $v"
else bad "packages/npm/package.json version is $v, want $CARGO_VERSION"; fi

# `npm ci` fails on a mismatch with package.json, and publish-npm.sh runs it.
v="$(json_field packages/npm/package-lock.json version)"
if [ "$v" = "$CARGO_VERSION" ]; then good "packages/npm/package-lock.json version = $v"
else bad "packages/npm/package-lock.json version is $v, want $CARGO_VERSION"; fi
